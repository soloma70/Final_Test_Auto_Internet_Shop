...
def test_us_filter_buy_fr_page(web_driver_desktop):
    """Тест UC "Я хочу найти и купить оправу бренда CARRERA, мужскую" осуществляет поиск по брендам и полу,
...
def test_us_filter_buy_sg_page(web_driver_desktop):
    """Тест UC "Я хочу найти и купить сонцезащитные очки бренда VOGUE, женские" осуществляет поиск по брендам и полу,
...
def test_us_cabinet_search_wishlist_cart_buy(web_driver_auth_desktop):
    ...
    только через ссылку.
    БАГ!!! В корзину добавляются суммы в целых грн., а в корзине по этим же позициям суммы в грн. и коп.,
