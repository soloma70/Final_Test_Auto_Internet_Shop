...
def test_edit_user_data(web_driver_auth_desktop):
    """Тест загружает кабинет, переходит на страницу редактирования данных,добавляе емейл,
    дату рождения, меняет язык по умолчанию на украинский, сохраняет данные, делает скриншот"""
...
def test_wishlist_user(web_driver_auth_desktop):
    """Тест загружает кабинет, переходит на страницу списка желаний, находит продукты
    соответственно тестовым наборам, проверяет получившийся список, удаляет его"""
...
def test_add_article_in_favorite_user(web_driver_auth_desktop):
    """Тест загружает кабинет, переходит на страницу блогов, выбирает рандомный блог, добавляет в избраннае,
    переходит на страницу Сохраненные статьи и проверяет название статьи среди сохраненных"""
